...
def handler(update, context):
    ...
    elif user_text =="오호!!좋은 점이 많은 것 같군..그럼 쥐띠와의 궁합을 알려줘": # 사용자가 보낸 메세지가 "안녕"이면?
    ...
    elif user_text =="헉..그건 또 이유가 뭔데": # 사용자가 보낸 메세지가 "뭐해"면?
        bot .send_message (chat_id =id , text ="느긋한 양띠는 쉬지 않고 일하는 소띠방식에 스트레스를 받게 되는데, 소띠는 자신의 감정을 솔직하게 말하지만, 소심한 양띠는 상처를 받고,서로가 서로를 피해다니는 관계이니 둘 관계는 멀어지게 될 거야") # 답장 보내기 
